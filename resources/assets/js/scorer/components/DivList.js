...
class DivListApp extends Component {
  ...
  render() {
    return [
      <div className="ui-body ui-body-a ui-corner-all" key={'CompHeader'}>
        <strong>Competition:</strong> {this.competitionName}
      </div>,
      <ul className="ui-listview listview-spacer" key={'CompList'}>
        {
          this.divs.map(item => {
...
class Division extends Component {
  render() {
    return <li><Link to={`/c/${this.props.compId}/d/${this.props.divId}`}
