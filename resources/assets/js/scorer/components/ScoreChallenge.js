...
class ScoreChallengeApp extends Component {
  ...
  prePopulateStateScores = () => {
    this.setState({
        'scores': this.props.challengeData[this.year][this.level][this.chalNum]
  ...
  render() {
    ...
    return [
      ...
      (chalData.random_lists && chalData.random_lists.length > 0) ?
        <RandomListPopup random_lists={chalData.random_lists} key='RandomListPopup' /> : null
      ,
      <ul className="ui-listview ui-listview-inset ui-corner-all ui-shadow" key='ScoreElements'>
        {
